...
/* BPF_PROG_TYPE_CGROUP_SOCK receives struct sock directly as its context. */
	   ...
	   int, addr_len)
{
	...
	if (addr->sa_family == AF_INET) {
		...
		if (((struct sockaddr_in *)addr)->sin_port != htons(0))
			return err;
		return __inet_bind(sk, addr, addr_len, true);
#if IS_ENABLED(CONFIG_IPV6)
		 ...
		 */
		return ipv6_bpf_stub->inet6_bind(sk, addr, addr_len, true);
#endif /* CONFIG_IPV6 */
	}
#endif /* CONFIG_INET */
...
static bool sk_lookup_is_valid_access(int off, int size,
				      ...
				      struct bpf_insn_access_aux *info)
{
	if (off < 0 || off >= sizeof(struct bpf_sk_lookup) || off % size != 0)
		return false;
	if (type != BPF_READ)
		return false;
					...
					u32 *target_size)
{
	...
	case bpf_ctx_range_till(struct bpf_sk_lookup,
		...
		off += bpf_target_off(struct in6_addr, s6_addr32[0], 4,
				      target_size);
		*insn++ = BPF_LDX_MEM(BPF_SIZEOF(void *), si->dst_reg,
				      si->src_reg,
...
int copy_bpf_fprog_from_user(struct sock_fprog *dst, sockptr_t src, int len)
{
	if (len != sizeof(struct sock_fprog))
		return -EINVAL;
	if (copy_from_sockptr(dst, src, len))
		return -EFAULT;
	return 0;
}
